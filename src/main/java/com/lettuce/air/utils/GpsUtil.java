...
public abstract class GpsUtil {
     ...
     * @param gg_lon
     */
    public static String[] gcj02_To_Bd09(String gg_lat, String gg_lon) {
		// 数据格式化
		DecimalFormat decimalFormat = new DecimalFormat();
		decimalFormat.applyPattern("#.000000");
        double x = Double.parseDouble(gg_lon);
        double y = Double.parseDouble(gg_lat);
        double z = Math.sqrt(x * x + y * y) + 0.00002 * Math.sin(y * pi);
        double theta = Math.atan2(y, x) + 0.000003 * Math.cos(x * pi);
        double bd_lon = z * Math.cos(theta) + 0.0065;
        double bd_lat = z * Math.sin(theta) + 0.006;
        String[] res = {decimalFormat.format(bd_lat), decimalFormat.format(bd_lon)};
        return res;
    }
