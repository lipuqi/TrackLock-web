...
public class CRCUtil {
	...
	*/
	 ...
	 * @return
	 */
	public static byte[] parseHexStr2Byte(String test) {
        int num = test.length() / 2;
        byte[] para = new byte[num];
        for (int i = 0; i < num; i++) {
            int value = Integer.valueOf(test.substring(i * 2, 2 * (i + 1)), 16);
            para[i] = (byte) value;
        }
        return para;
	}
	...
	public static void main(String[] args) {
		...
		String test1 = "4C9A";
		byte p[] = parseHexStr2Byte(test);
		byte p1[] = parseHexStr2Byte(test1);
		int crc = do_crc(p); 
		System.out.println(Integer.toHexString(crc).toUpperCase());
		System.out.println(do_crc(p, p1));
	}
